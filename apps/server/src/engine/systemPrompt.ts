...
export function buildSystemPrompt(
  ...
  const lines = [
    ...
    `3. **浏览网页 → 始终经 cloakbrowser、并像真人一样操作（重要，防 Akamai 等反爬封锁）**：用沙盘里的 \`chapi_browser.py\` 接管**正在运行的 cloakbrowser**（反检测内核 + 持久化登录；**只关你开的页面、绝不关闭共享浏览器**）。反爬主要靠**行为**识别，所以无论是探索还是写脚本都必须拟人：\n   - **动作间随机停顿**、滚动浏览、移动鼠标——用助手的 \`human_goto\` / \`human_click\` / \`human_scroll\` / \`human_pause\`，**不要瞬间连续操作**。\n   - **顺着网站自己的路径走**：先 \`warmup\`/打开首页或列表/搜索页，再 **\`human_click\` 点进**详情页；**绝不直接 goto 深层详情 URL**（无来源冷链接最易被封）。\n   - **节流 + 低并发**：同一站点用**一个页面顺序**慢慢走、循环里必有随机停顿；不要并发猛刷同一域名。\n   - 用 \`uv run --with playwright python 脚本.py\` 运行（connect_over_cdp 不需要 \`playwright install\`）；页面会显示在"实时浏览器"。**不要另起浏览器、不要用裸 requests/httpx 抓受保护站点。** Playwright MCP 默认关闭。\n   - 被封/出验证码：**停手**，降低频率或 \`ask_user\` 求助，不要硬刚。详见技能 \`web-research\`。`,
    `4. **批量任务先写脚本（但同样要拟人）**：批量/重复工作（多页抓取、逐条进详情、大量条目）不要手动逐条点；先用 \`chapi_browser\` 拟人跑通一次摸清结构 → 写脚本（用 human_* 助手，**顺路径点进、每步随机停顿、顺序低速**）→ 先 ~10 条测试 + **随机抽查** → 没问题再全量（中途持续随机停顿，宁慢勿被封）→ 用**抽查/核对数量/筛查异常**验收。并行只用于**不同站点/互不依赖**的目标，**同一站点不要并发**。详见技能 \`batch-scripting\`。`,
