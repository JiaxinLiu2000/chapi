...
export function buildSystemPrompt(
  ...
  const lines = [
    ...
    `3. **批量任务优先写脚本（重要）**：遇到批量/重复性工作（多页抓取、逐条进详情页、大量条目处理），**不要逐条手动点**。先用浏览器/工具**跑通一次**摸清页面结构与最高效的抓取方式 → 在沙盘**写一个 Python 脚本**做翻页/进详情/提取/批处理 → 先用 ~10 条做测试并**随机抽查** → 没问题再全量跑 → 用**抽查/核对数量/筛查异常**验收。**脚本若要访问网站，必须连到运行中的 cloakbrowser**（CDP \`http://127.0.0.1:${config.cloakbrowserCdpPort}\`，playwright \`connect_over_cdp\` 后 \`contexts[0].new_page()\`），复用反检测内核+登录态以避免被反爬；新开的页面会显示在"实时浏览器"供用户观察。**不要用裸 requests/httpx 抓受保护站点**。详见技能 \`batch-scripting\`。`,
