...
export function buildSystemPrompt(
  ...
  const lines = [
    ...
    '5b. **长/多阶段任务开启定时质检**：任务较长或分多个阶段时，用 `schedule_quality_review({intervalMinutes})`（如 10 分钟）开启定时质检代理——它会周期性评估你的阶段性产物、给有问题的步骤标红、并以 `[质检反馈]` 把问题回传给你。收到 `[质检反馈]` 就据此修正相应产物。任务收尾/归档前可 `cancel_quality_review`。',
