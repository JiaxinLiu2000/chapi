...
- 根因:v0.1.61 加了"滚动到顶部时自动加载更早消息"，同时保留了原来"新内容用平滑滚动滚到底部"的逻辑。
  但打开一个会话时，初始滚动位置是 0(顶部)，而滚到底部用的是 `scrollTo({behavior:'smooth'})`——
  这个动画会让 `scrollTop` 依次经过 0 到底部之间的**所有中间值**，其中当然包括"接近顶部"的那些值。
  于是每次一打开会话，这个平滑滚动动画自己触发的 `scroll` 事件，就会在滚到底部的路上被误判成
  "用户往上翻到了顶部"，从而多此一举地又加载了一批更早的消息，并且把滚动位置重新计算/跳到了
  刚加载的这批旧消息附近——结果就是停在了"很多条对话之前的位置"，而不是最新消息。
- 修复:每个会话**第一次**加载时，直接瞬间跳到底部(不用动画)，从根上避免滚动条经过"接近顶部"的
  中间状态；只有后续新消息/流式输出时才继续用平滑滚动跟随。另外加了一道保险：加载更早消息的判断，
  只有在完成过一次"打开会话跳到底部"之后才会生效，避免初始化阶段的任何滚动事件被误判。
...
- 根因(有真实日志佐证):v0.1.56 每次收到限额都会把该账号的"限额时间"重新打成当前时间。但如果
  实际额度还没恢复、重试自然又会失败，每次失败都重新打点，预计恢复时间就被不断推迟——
  19:15 主账号限额本该 00:15 恢复，结果因为期间又被打了两次点，硬是被推到了 01:29。
  用户反馈"token 都刷新了还是显示两个账号均已达限额"正是这个原因：账号确实还没恢复
  （重试时 Anthropic 仍返回限额），但即便恢复了，只要还在旧的冷却窗口内重试，时间也会被继续推后。
- 修复:同一账号的冷却窗口还没结束时，重复限额不再重新打点，只保留本轮最早那次的时间，
  确保预计恢复时间是"从第一次限额起 + 冷却小时数"，不会因为重试而不断顺延。
- 顺带把这条会话被意外推迟的两个时间戳改回了本轮最早的记录值。
